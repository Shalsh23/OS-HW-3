...
__device__ unsigned long computeHash(char *str)
{
#if 1
...
// my stuff 
__device__ void initHashTable(MyHashMapElement *hme, int tablerange, int subrange)
{
...
__device__ bool insertElement(MyHashMapElement *hme, unsigned long key, int keyshift, int bucketsize, int strlength, int initvalue)
{
...
__device__ int findElement(MyHashMapElement *hme, unsigned long key, int keyshift, int bucketsize, int strlength)
{
  unsigned long newkey = key & ( (1 << keyshift) - 1 );  // clear the MSBs
  MyHashMapElement *bucket = &hme[newkey * bucketsize];
  int numEleInBucket = bucket->countInBuc;
